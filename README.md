...
**Студийный рекордер** — это автоматизированная система для записи, контроля и постобработки RTSP-аудио и RTSP-видео по аудиториям и комнатам.

Проект создавался для обеспечения надежной записи образовательных или студийных процессов. Система берет на себя весь цикл работы с медиа: от захвата потоков с камер и источников звука до автоматического мультиплексирования, синхронизации и формирования итоговых файлов. Продукт предоставляет удобный веб-интерфейс для управления записями, а также встроенные инструменты мониторинга состояния системы.

---

## Оглавление

1. [Стадия разработки проекта](#стадия-разработки-проекта)
2. [Состав системы](#состав-системы)
3. [Быстрый запуск](#быстрый-запуск)
4. [Адреса сервисов](#адреса-сервисов)
5. [Как сервисы связаны](#как-сервисы-связаны)
6. [Подробное описание компонентов](#подробное-описание-компонентов)
7. [Конфигурация данных](#конфигурация-данных)
8. [Импорт конфигурации из CSV](#импорт-конфигурации-из-csv)
9. [Запись комнаты](#запись-комнаты)
10. [Файлы записи](#файлы-записи)
11. [Мониторинг](#мониторинг)
12. [Логи](#логи)
13. [Когда использовать каждый сервис](#когда-использовать-каждый-сервис)
...
### ✅ Этап 5: MVP (Minimal Viable Product) — Прошедший этап

Система работает автономно и может быть использована без участия разработчика. Реализованы все базовые требования:

- **Веб-интерфейс (Frontend):** создание и редактирование конфигураций комнат.
- **Работа с потоками:** выбор аудиоисточника из списка записываемых камер.
- **Синхронизация:** поддержка NTP.
- **Управление:** запуск и остановка записи вручную.
- **Постобработка:** автоматическое мультиплексирование (сведение всех видео и выбранного аудио в единый файл) и автозапуск склейки после окончания записи.
- **Надежность и мониторинг:** автозапуск сервисов через Docker Compose, мониторинг через Prometheus (статус, ошибки, нагрузка) и алертинг через Alertmanager/Grafana.
- **Документация:** наличие руководств пользователя и разработчика.

### 🚀 Этап 6: MUP (Minimal Usable Product) — Текущий этап

Цель текущего этапа — полноценное внедрение в тестовую среду (например, в лаборатории сетевых видеотехнологий) и передача полного контроля над продуктом реальным пользователям.

**Планы и задачи на этап MUP:**

1. **Тестирование:** Проверка на реальном пользовательском взаимодействии и оперативное исправление обнаруженных ошибок.
2. **Внесение правок:** Увеличение количества агентов до 10
3. **Обратная связь:** Получение развернутого пользовательского отзыва о реальном использовании (удобство интерфейса, точность синхронизации, стабильность) и дополнение документации на его основе.
4. **Масштабирование:** При необходимости увеличение числа пишущих агентов для распределения нагрузки.
5. **Продвижение:** Создание маркетинговых материалов (лендинг, продуктовый ролик, презентация/раздатка), адаптированных под целевую аудиторию.
...
- свободные порты `3000`, `5173`, `5433`, `8082`, `8083`, `8084`, `8085` , `8086`, `8087`, `8088`, `8089`, `8091`, `8082`, `8090`, `9090`;
- каталог `videos` для записей.

Скачиваем

```bash
git clone --branch mvp https://git.miem.hse.ru/19102/recorder
```
...
| Agent 3        | http://localhost:8084                    | Третий recorder agent               |
| Agent 4        | http://localhost:8085                    | Четвертый recorder agent            |
| Agent 5        | http://localhost:8086                    | Пятый recorder agent                |
| Agent 6        | http://localhost:8087                    | Шестой recorder agent               |
| Agent 7        | http://localhost:8088                    | Седьмой recorder agent              |
| Agent 8        | http://localhost:8089                    | Восьмой recorder agent              |
| Agent 9        | http://localhost:8091                    | Девятый recorder agent              |
| Agent 10       | http://localhost:8092                    | Десятый recorder agent              |
...
Внутри Docker-сети сервисы обращаются друг к другу по именам из `docker-compose.yaml`: `master`, `agent-1`, `agent-2`, `agent-3`, `agent-4`, `agent-5`, `agent-6`, `agent-7`, `agent-8`, `agent-9`, `agent-10`, `postgres`, `prometheus`, `grafana`.
...
- отдаёт состояние потоков, комнаты и метрики Prometheus. Обычно один agent обслуживает одну комнату / один набор потоков в один момент времени.
...
- после остановки запускает `postprocess-room` для каждого video-потока. Master нужен для того, чтобы не обращаться к каждому agent напрямую.
...
- склеивает видеосегменты и вставляет чёрные видео-заглушки на разрывах;
- склеивает аудиосегменты и вставляет тишину на разрывах связи;
...
PostgreSQL инициализируется файлом `recorder/master/backend/db/init.sql`:
...
('agent-3', 'http://agent-1:8084'),
('agent-4', 'http://agent-2:8085'),
('agent-5', 'http://agent-1:8086'),
('agent-6', 'http://agent-2:8087'),
('agent-7', 'http://agent-1:8088'),
('agent-8', 'http://agent-2:8089'),
('agent-9', 'http://agent-1:8091'),
('agent-10', 'http://agent-2:8092')
```
...
- имена потоков генерируются автоматически (`V1`, `V2`, ... и `Audio1`, `Audio2`, ...);
...
При запуске через Docker Compose записи доступны на хосте в: `./videos`
Master передаёт agent путь назначения через конфигурацию комнаты (`DESTINATION: /app/videos`).
...
В `docker-compose.yaml` добавлены Prometheus (`http://localhost:9090`) и Grafana (`http://localhost:3000`).
...
- `agent-3:8084/metrics`;
- `agent-4:8085/metrics`;
- `agent-5:8086/metrics`;
- `agent-6:8087/metrics`;
- `agent-7:8088/metrics`;
- `agent-8:8089/metrics`;
- `agent-9:8091/metrics`;
- `agent-10:8092/metrics`;

Правила алертов лежат в `prometheus/alerts/recorder.yml`. Дашборды Grafana лежат в `grafana/provisioning/dashboards`.
...
В `agent` и `master` поддерживается переменная окружения `LOG_LEVEL` (по умолчанию `info`).
Для подробной диагностики (вывод технических access logs, CORS logs, штатных RTSP-событий) можно временно включить `LOG_LEVEL: debug` в `docker-compose.yaml`.
...
docker compose logs -f agent-4
docker compose logs -f agent-5
docker compose logs -f agent-6
docker compose logs -f agent-7
docker compose logs -f agent-8
docker compose logs -f agent-9
docker compose logs -f agent-10
